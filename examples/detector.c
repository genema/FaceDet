...
int init(char * datacfg, char * cfgfile, char * weightfile, REQS * req)
{
    // Detector network structure and weights initialization
    //
    list * options   = read_data_cfg(datacfg);
    char * name_list = option_find_str(options, "names", "data/names.list");
    req->names       = get_labels(name_list);
    req->mynet = parse_network_cfg(cfgfile);
    if (weightfile) load_weights(req->mynet, weightfile);
    set_batch_network(req->mynet, 1);
    req->alphabet = load_alphabet();
    return 1;
}
